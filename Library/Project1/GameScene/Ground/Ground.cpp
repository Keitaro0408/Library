...
Ground::Ground()
{
	m_Scale = Lib::VECTOR3(1.f, 1.f, 1.f);
	...
	SINGLETON_INSTANCE(Lib::FbxFileManager).LoadFbxModel("map.fbx", &m_ModelIndex);
...
bool Ground::WriteConstantBuffer()
{
...
bool Ground::CreateVertexLayout()
{
...
bool Ground::CreateDepthStencilState()
{
	// 深度ステンシルステートの設定.
...
bool Ground::CreateConstantBuffer()
{
	// 定数バッファの設定.
...
void Ground::VertexLayoutSetup()
{
	ID3D11DeviceContext* pContext = SINGLETON_INSTANCE(Lib::DX11Manager).GetDeviceContext();
	pContext->IASetInputLayout(m_pVertexLayout);
}

void Ground::DepthStencilStateSetup()
{
	ID3D11DeviceContext* pContext = SINGLETON_INSTANCE(Lib::DX11Manager).GetDeviceContext();
...
void Ground::ConstantBufferSetup()
{
	ID3D11DeviceContext* pContext = SINGLETON_INSTANCE(Lib::DX11Manager).GetDeviceContext();
